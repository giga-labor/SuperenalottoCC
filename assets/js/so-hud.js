...
window.SECC_HUD = (function () {
  ...
  const SO_LABELS = Object.freeze({
    root: 'SOH', statistiche: 'SO Statistiche', archivio: 'SO Archivio',
    ranking: 'SO Ranking', algoritmi: 'SO Algoritmi', spotlight: 'SO Spotlight',
    super6nn: 'SO Super6NN', oracle: 'SO Oracle', about: 'SO Info', policy: 'SO Policy'
  });
  ...
  function updateTravelPctOverlay({ autoPilot, camPos, navTrans, thrustBoost, BOOST_HYPER_LEVEL }) {
  ...
  function coordTick({ phase, camPos, activeNodeId }) {
    ...
    if (_sorDistEl && _sorAzEl && _sorBrgEl) {
      // calcCoordToDockedSO (inline)
      let s = null;
      if (activeNodeId && MENU_DEFS[activeNodeId] && NAV_NODES[activeNodeId]) {
  ...
  return {
    ...
    // SO HUD
    refreshSoSelectDarhLabels, syncSoHud,
    // DOM layout
